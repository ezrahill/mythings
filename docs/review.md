...
Solid implementation of the plan and design — data model, repository layer, and component structure all match `docs/plan.md` closely, and the design tokens/palette match `docs/design.md` exactly. 135 tests pass, build is clean. Ready to merge after the two must-fixes (Today/Evening duplicate rendering, overdue-recurrence advancement) — both are small, well-isolated fixes in code already covered by adjacent tests.
